...
int main() {
  ...
  CHECK_ADBC(AdbcDatabaseSetOption(&database, "driver", "snowflake", &error));
  CHECK_ADBC(AdbcDatabaseSetOption(&database, "username", "USER", &error));

  // For username/password authentication:
  CHECK_ADBC(AdbcDatabaseSetOption(&database, "adbc.snowflake.sql.auth_type",
                                   "auth_snowflake", &error));
  CHECK_ADBC(AdbcDatabaseSetOption(&database, "password", "PASS", &error));

  // For JWT authentication, uncomment these lines instead:
  // CHECK_ADBC(AdbcDatabaseSetOption(&database, "adbc.snowflake.sql.auth_type",
  //                                  "auth_jwt", &error));
  // CHECK_ADBC(AdbcDatabaseSetOption(
  //     &database, "adbc.snowflake.sql.client_option.jwt_private_key",
  //     "/path/to/rsa_key.p8", &error));

  CHECK_ADBC(AdbcDatabaseSetOption(&database, "adbc.snowflake.sql.account",
                                   "ACCOUNT-IDENT", &error));
  CHECK_ADBC(AdbcDatabaseSetOption(&database, "adbc.snowflake.sql.db",
                                   "SNOWFLAKE_SAMPLE_DATA", &error));
  CHECK_ADBC(AdbcDatabaseSetOption(&database, "adbc.snowflake.sql.schema",
                                   "TPCH_SF1", &error));
  CHECK_ADBC(AdbcDatabaseSetOption(&database, "adbc.snowflake.sql.warehouse",
                                   "MY_WAREHOUSE", &error));
  CHECK_ADBC(AdbcDatabaseSetOption(&database, "adbc.snowflake.sql.role",
                                   "MY_ROLE", &error));
  ...
  CHECK_ADBC(AdbcStatementSetSqlQuery(&statement,
                                      "SELECT * FROM CUSTOMER LIMIT 5", &error));
  CHECK_ADBC(
