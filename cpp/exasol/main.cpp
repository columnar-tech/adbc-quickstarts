...
int main() {
  ...
  CHECK_ADBC(AdbcDatabaseSetOption(&database, "driver", "exasol", &error));
  CHECK_ADBC(AdbcDatabaseSetOption(
      &database, "uri", "exasol://sys:exasol@localhost:9563/?tls=true&validateservercertificate=0",
      &error));
  ...
  CHECK_ADBC(
      AdbcStatementSetSqlQuery(&statement, "SELECT * FROM DEMO.GAMES", &error));
  CHECK_ADBC(
