...
int main() {
  ...
  CHECK_ADBC(AdbcDatabaseSetOption(&database, "uri", "grpc://localhost:10000",
                                   &error));
  CHECK_ADBC(AdbcDatabaseSetOption(
      &database, "adbc.flight.sql.authorization_header",
      "Bearer io.deephaven.authentication.psk.PskAuthenticationHandler "
      "YOUR_PASSWORD_HERE",
      &error));
  ...
  CHECK_ADBC(
      AdbcStatementSetSqlQuery(&statement, "SELECT 42 AS foo", &error));
  CHECK_ADBC(
