...
# Connecting C++ and GreptimeDB with ADBC

## Instructions

> [!TIP]
> If you already have a GreptimeDB instance running, skip the steps to set up and clean up GreptimeDB.
...
2. Start a GreptimeDB instance:

    ```sh
    docker run -d --rm --name greptime -p 4002:4002 greptime/greptimedb:v1.0.0-beta.4 standalone start --mysql-addr 0.0.0.0:4002
    ```
...
2. Stop the Docker container running GreptimeDB:
