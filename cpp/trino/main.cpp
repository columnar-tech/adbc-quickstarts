...
int main() {
  ...
  CHECK_ADBC(AdbcDatabaseSetOption(&database, "driver", "trino", &error));
  CHECK_ADBC(AdbcDatabaseSetOption(
      &database, "uri", "http://user@localhost:8080?catalog=tpch&schema=tiny",
      &error));
  ...
  CHECK_ADBC(AdbcStatementSetSqlQuery(
      &statement,
      "SELECT nationkey, name, regionkey FROM tpch.tiny.nation LIMIT 5",
      &error));
  CHECK_ADBC(
