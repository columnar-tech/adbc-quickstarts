...
int main() {
  ...
  CHECK_ADBC(AdbcDatabaseSetOption(&database, "driver", "bigquery", &error));
  CHECK_ADBC(AdbcDatabaseSetOption(&database, "adbc.bigquery.sql.project_id",
                                   "my-gcp-project", &error));
  CHECK_ADBC(AdbcDatabaseSetOption(&database, "adbc.bigquery.sql.dataset_id",
                                   "bigquery-public-data", &error));
  ...
  CHECK_ADBC(AdbcStatementSetSqlQuery(
      &statement,
      "SELECT word, corpus FROM bigquery-public-data.samples.shakespeare WHERE "
      "word_count = 1 ORDER BY RAND() LIMIT 5;",
      &error));
  CHECK_ADBC(
