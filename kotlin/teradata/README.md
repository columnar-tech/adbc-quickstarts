...
# Connecting Kotlin and Teradata with ADBC
...
1.  Install the ADBC driver for Teradata:

    ```sh
    dbc install teradata
    ```

1.  Download and install the [Teradata Tools and Utilities (TTU)](https://downloads.teradata.com/). Select "Tools and Utilities" and choose the package for your platform. Install to the default location:
    -   Linux: `/opt/teradata`
...
1.  Set `LD_LIBRARY_PATH` (Linux), `DYLD_LIBRARY_PATH` (macOS), or `PATH` (Windows) to make sure the TTU libraries are discoverable by your application.
