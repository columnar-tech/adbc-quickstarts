...
# Connecting Kotlin and chDB with ADBC
...
1.  Customize the `main` function in `Main.kt`
    -   Change the SQL SELECT statement in `stmt.setSqlQuery`, or keep it set to `SELECT * FROM 'games.parquet';` to query the Parquet file included with this example
