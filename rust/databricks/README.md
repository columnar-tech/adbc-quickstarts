...
# Connecting Rust and Databricks with ADBC
...
2. Customize `src/main.rs` as needed:
    - Change the connection arguments in `opts`:
        - `OptionDatabase::Uri` is the URI for your Databricks instance. It uses the following syntax: `databricks://token:<personal-access-token>@<server-hostname>:<port-number>/<http-path>`.
